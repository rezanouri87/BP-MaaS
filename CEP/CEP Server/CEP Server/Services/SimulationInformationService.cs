...
namespace CEP.Server.Adaptor
{
    ...
    public class SimulationInformationService : ISimulationInformationService
    {
        ...
        public Boolean SubscribeEventData()
        {
            Log.Info("Client subscribed to get event data");

            epService.EPAdministrator.GetStatement("_Hide_MonitoreAllEventChanges_UsedForSimulation").Events += OnEventChange;
        ...
        private void OnEventChange(object sender, UpdateEventArgs e)
        {
            var dict = e.NewEvents.FirstOrDefault().Underlying as Dictionary<String, object>;

            try
            {
                Log.Debug("Send OnEventChange Event");
                client.ReceiveEventChange(dict);
            }
